...
## 🆕 v2.0 更新：雙模式 (Hybrid Mode)

為了同時滿足 **單機便利性** 與 **伺服器多人部署** 需求，v2.0 引入了雙模式架構。

### 1. Local Mode (預設模式)
- **適用場景**：個人 Windows 電腦、本地開發。
- **行為**：
  - 自動讀寫 `tasks.json`，所見即所得。
  - 資料會持久化儲存在本機。
- **啟動方式**：直接執行 `streamlit run gantt_app.py` 或 `run_gantt.bat`。

### 2. Web Mode (無痕模式)
- **適用場景**：Ubuntu Server、Docker、多人同時使用環境。
- **行為**：
  - **不自動存檔**：程式完全不讀寫伺服器硬碟，確保伺服器乾淨。
  - **資料隔離**：每位使用者的狀態僅存在於瀏覽器記憶體中，互不干擾。
  - **上傳/下載**：使用者必須透過側邊欄的「下載專案檔」來保存進度，下次使用時「上傳專案檔」還原。
- **啟動方式**：
  ```bash
  # 請務必加上 -- --web 參數
  streamlit run gantt_app.py -- --web
  ```

### 3. 技術變更
- **In-Memory 生成**：PPTX 檔案現在直接在記憶體中生成並串流下載，伺服器端不再產生 `.pptx` 暫存檔，無需設置清理腳本。